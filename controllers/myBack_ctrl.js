...
var app = angular.module('app', ['ngRoute'])
    .controller('MyBackCtrl', ['$scope','$http','$anchorScroll', function ($scope, $http, $anchorScroll) {
        $anchorScroll();
        $scope.state=6;
        $scope.menu=0;
        $scope.data={};
                ...
                if(result.error >0) {
                    ...
                    angular.forEach($scope.orders, function (value, item) {
                        if($scope.orders[item].status == 0 ){
                            ...
                            $scope.orders[item].status = '店家已收货';
                        }else if($scope.orders[item].status == 4) {
