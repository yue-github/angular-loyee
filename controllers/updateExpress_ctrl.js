...
var app = angular.module('app', ['ngRoute'])
    .controller('updateExpressCtrl',['$scope','$http','$anchorScroll','$rootScope', '$location',function ($scope, $http, $anchorScroll, $rootScope, $location) {
        $anchorScroll();
        $scope.shopmenu=6;
        $scope.state=0;

        $scope.data={};
        var templateId = $location.search().id;
        $scope.data.templateId=templateId;
        getLogistics($scope,$http,$scope.data);
        //添加指定地区的运送方式
        var object={};
        ...
        $scope.deletelist=function(p_k,k){
            ...
            $scope.data.expressType=$scope.data.expressType;
        }
        $scope.cityshow=0;
       ...
       getProvinces($scope,$http);
        ...
        $scope.submit=function(){
            ...
            if($scope.butflag){
                $scope.butflag=false;
                updateLogisticsTemplate($scope,$http,template);
            }
        }
        //显示城市层
        $scope.shownum=-1;
