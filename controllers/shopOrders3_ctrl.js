...
var app = angular.module('app', ['ngRoute'])
    .controller('ShopOrders3Ctrl',['$scope','$http','$anchorScroll','$location','$filter', function ($scope, $http, $anchorScroll, $location,$filter) {
        $anchorScroll();
        $scope.menustate=3;
        $scope.state=3;
        $scope.data={};
        ...
        $scope.export=function(){
            ...
            $http.post(baseUrl + 'pc/shopOrders/exportOrders2?status=3',$scope.filter ).success(function (result) {
                if(result.error >0) {
                    alert(result.errmsg);
                } else {
