...
var app = angular.module('app', ['ngRoute', 'ngFileUpload'])
	.controller('SalesPromotionCtrl8', ['$scope', '$http', '$anchorScroll', '$location', '$filter', 'Upload', function($scope, $http, $anchorScroll, $location, $filter, Upload) {
		$anchorScroll();
		$scope.menustate = 6;
		$scope.state = 6;
		$scope.shopmenu = 4;
		...
		var getPromotion = function() {
			...
			$scope.data.length = 10;
			$scope.data.offset = 0;
			$http.post(baseUrl + 'pc/promotionKanjia/many', $scope.data).success(function(result) {
				if(result.error > 0) {
		...
		$scope.submit = function() {
			...
			$scope.data.favourable = $("input[name='mj_favourable']").val();
			$scope.data.id = $scope.yue_id;
			...
			if($scope.formTitle == '添加') {
				...
				$http.post(baseUrl + "pc/promotionKanjia/create", data).success(function(result) {
					if(result.error > 0) {
			...
			if($scope.formTitle == '修改') {
				...
				$http.post(baseUrl + "pc/promotionKanjia/update", data).success(function(result) {
					if(result.error > 0) {
						$byLayer.msg(result.errmsg, 'failed');
					} else {
						getPromotion();
		...
		$scope.update = function(id) {
			...
			$scope.data = {};
			$scope.formTitle = "修改";
			$http.post(baseUrl + 'pc/promotionKanjia/get', {
				id: id,
				promotionType: 8
		...
		$scope.delete = function(id, hint) {
			...
			$byLayer.confirm("确定删除：" + hint, function() {
				$http.post(baseUrl + 'pc/promotionKanjia/delete', $scope.data).success(function(result) {
					if(result.error > 0) {
