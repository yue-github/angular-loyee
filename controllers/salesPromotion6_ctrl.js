...
var app = angular.module('app', ['ngRoute', 'ngFileUpload'])
	.controller('SalesPromotionCtrl6', ['$scope', '$http', '$anchorScroll', '$location', '$filter', 'Upload', function($scope, $http, $anchorScroll, $location, $filter, Upload) {
		$anchorScroll();
		$scope.menustate = 6;
		$scope.state = 6;
		$scope.shopmenu = 4;
		...
		var getPromotion = function() {
			...
			$scope.data.length = 10;
			$scope.data.offset = 0;
			$http.post(baseUrl + 'pc/promotionBaoyou/many', $scope.data).success(function(result) {
				if(result.error > 0) {
		...
		$scope.submit = function() {
			...
			if($scope.formTitle == '添加') {
				...
				$http.post(baseUrl + "pc/promotionBaoyou/create", data).success(function(result) {
					if(result.error > 0) {
			...
			if($scope.formTitle == '修改') {
				...
				$http.post(baseUrl + "pc/promotionBaoyou/update", data).success(function(result) {
					if(result.error > 0) {
						$byLayer.msg(result.errmsg, 'failed');
					} else {
						getPromotion();
		...
		$scope.update = function(id) {
			...
			$scope.data = {};
			$scope.formTitle = "修改";
			$http.post(baseUrl + 'pc/promotionBaoyou/get', {
				id: id,
				promotionType: 1
