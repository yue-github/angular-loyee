...
var getProductInfo=function($scope,$http,productId,$sce){
    $http.post(baseUrl + 'pc/product/productInfo', {id: productId}).success(function (result) {
        if (result.error == 0)  {
            ...
            if($scope.product.prod_type == 1)
                $scope.price = $scope.product.actualUnitPrice;
            ...
            saveCustomerLook($scope, $http, $scope.product);
        }
    });
}
...
//加入购物车
var addCart = function ($scope,$http,productId,amount,priceId,vip_is) {
    $http.post(baseUrl + 'pc/order/addCart', {id: productId, amount: amount, priceId:priceId,vip_is}).success(function (result) {
        if (result.error == 0) {
...
var getHotelPrice=function($scope,$http,data){
    $http.post(baseUrl + 'pc/product/getHotelPrice',data).success(function (result) {
        // 用于旅游返回结果处理，本来返回的是error:1,后台接口需要改动,错误为一说明可能是旅游
        if(result.error == 1){
            $byLayer.msg('导游方会与您具体商议时间，您可忽略时间直接下单', 'warning');
            $scope.is_correct_hotel = true;
            return;
        }
        if(result.error > 0) {
